...
class BaseModel:
    ...
    def init_model(self):
        ...
        if self.hazard == 'SLE':
            model.beam_element(1, 1, 2, a_w, self.args.e, self.args.rot1 * i_w)
            model.beam_element(2, 3, 4, a_c, self.args.e, self.args.rot1 * i_c)
        else:
            model.beam_element(1, 1, 2, a_w, self.args.e, self.args.rot2 * i_w)
            model.beam_element(2, 3, 4, a_c, self.args.e, self.args.rot2 * i_c)
    ...
    def forward(self):
        ...
        if self.hazard == 'SLE':
            model.ground_motion(int(self.args.gm_dir), self.gm_data, 1 / self.args.fs, self.args.factor1)
        else:
            model.ground_motion(int(self.args.gm_dir), self.gm_data, 1 / self.args.fs, self.args.factor2)
        ...
        if self.hazard == 'SLE':
            alpha = 2 * f1 * f2 * self.args.xi1 / (f1 + f2)
            beta = 2 * self.args.xi1 / (f1 + f2)
        else:
            alpha = 2 * f1 * f2 * self.args.xi2 / (f1 + f2)
            beta = 2 * self.args.xi2 / (f1 + f2)
