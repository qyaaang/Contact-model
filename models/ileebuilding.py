...
class ILEEBuilding:
    ...
    def init_model(self):
        ...
        f_p = self.args.f_pi * (np.pi * self.args.d ** 2 / 4)  # PT force
        m_p = f_p / 9800  # Equivalent mass due to PT
        m_w_all = m_w + m_p / 2
        m_c_all = m_c + self.args.mb / 2
        model.mass(1, 0.5 * m_c_all, 0.5 * m_c_all, 0.5 * m_c_all)
        model.mass(2, 1.0 * m_c_all, 1.0 * m_c_all, 1.0 * m_c_all)
        ...
        model.mass(7, 0.5 * m_c_all, 0.5 * m_c_all, 0.5 * m_c_all)
        model.mass(8, 1.0 * m_c_all, 1.0 * m_c_all, 1.0 * m_c_all)
        model.mass(9, 0.5 * m_c_all, 0.5 * m_c_all, 0.5 * m_c_all)
        ...
        model.hertz_contact_2(1, self.params[0],
                              ...
                              self.args.mu, self.args.g_tol)
        model.hertz_contact_2(2, self.params[1],
                              self.args.k_c1, self.params[2] * self.args.k_c1,
                              self.args.c_c1, self.params[3] * self.args.c_c1,
                              self.args.mu, self.args.g_tol)
        # model.beam_element(1, 1, 2, a_c, self.args.e, 0.6 * i_c)
        # model.beam_element(2, 2, 3, a_c, self.args.e, 0.5 * i_c)
        # model.beam_element(3, 4, 5, a_w, self.args.e, 0.2 * i_w)
        # model.beam_element(4, 5, 6, a_w, self.args.e, 0.2 * i_w)
        # model.beam_element(5, 7, 8, a_c, self.args.e, 0.6 * i_c)
        # model.beam_element(6, 8, 9, a_c, self.args.e, 0.5 * i_c)
        model.beam_element(1, 1, 2, a_c, self.args.e, self.params[4] * i_c)
        model.beam_element(2, 2, 3, a_c, self.args.e, self.params[4] * i_c)
        model.beam_element(3, 4, 5, a_w, self.args.e, self.params[5] * i_w)
        model.beam_element(4, 5, 6, a_w, self.args.e, self.params[5] * i_w)
        model.beam_element(5, 7, 8, a_c, self.args.e, self.params[4] * i_c)
        model.beam_element(6, 8, 9, a_c, self.args.e, self.params[4] * i_c)
