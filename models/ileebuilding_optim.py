...
class ILEEBuilding:
    ...
    def init_model(self):
        ...
        model.node(2, -self.params[0], self.params[7])
        model.node(3, -self.params[1], self.params[7] * 2)
        model.node(4, 0, 0)
        model.node(5, 0, self.params[7])
        model.node(6, 0, self.params[7] * 2)
        model.node(7, self.params[0], 0)
        model.node(8, self.params[0], self.params[7])
        model.node(9, self.params[1], self.params[7] * 2)
        m_w = self.args.rou * self.args.h_w * self.args.b_w * self.params[7]  # Lumped mass of wall
        m_c = self.args.rou * self.args.h_c * self.args.h_c * self.params[7]  # Lumped mass of column
        f_p = self.params[10] * (np.pi * self.params[11] ** 2 / 4) * self.args.num_bar  # PT force
        m_p = f_p / 9800  # Equivalent mass due to PT
        m_w_all = m_w + m_p / 2
        m_c_all_1 = m_c + self.params[8] / 2
        m_c_all_2 = m_c + self.params[9] / 2
    ...
    def forward(self):
        ...
        freq = mode()
        f1, f2 = freq[0], freq[1]
        alpha = 2 * f1 * f2 * self.params[12] / (f1 + f2)
        beta = 2 * self.params[12] / (f1 + f2)
